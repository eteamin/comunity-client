...
class OverScrollEffect(KineticEffect):
    ...
    def reset(self, pos):
        ...
        if self.history:
            ...
            super(OverScrollEffect, self).start(val, None)
    ...
    def start(self, val, t=None):
        self.is_manual = True
        self.displacement = 0
        return super(OverScrollEffect, self).start(val, t)

    def update(self, val, t=None):
        self.displacement += abs(val - self.history[-1][1])
        return super(OverScrollEffect, self).update(val, t)
    ...
    def change_opacity(self, *args):
        self.target_widget.children[-1].opacity = 1
